...
export function CourseTileView({ course, viewDetailsLabel }: CourseTileViewProps) {
  return (
    <Card className="group flex h-full flex-col overflow-hidden rounded-[10px] bg-surface shadow-[0_7px_20px_rgba(17,24,39,0.05)] hover:-translate-y-1 hover:border-primary/15 hover:shadow-[0_18px_40px_rgba(29,23,213,0.10)]">
      ...
      <div className="flex flex-1 flex-col px-4 pb-4 pt-4">
        ...
        <div className="mt-5 flex items-center justify-between gap-4">
          <p className="text-[1rem] font-black text-primary">{course.price}</p>
          <Link href={course.href} className="inline-flex items-center gap-1.5 rounded-[7px] bg-primary/6 px-4 py-2 text-[0.72rem] font-bold text-primary transition duration-200 hover:bg-primary hover:text-white">
            {viewDetailsLabel}
