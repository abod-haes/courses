...
import { ApiError, clearWebsiteClientSession } from "@/shared/api/client";
...
export function AuthFormCard({ mode, copy }: AuthFormCardProps) {
  ...
  useEffect(() => {
    if (mode === "login" && searchParams.get("sessionExpired") === "1") {
      ...
      <div className="relative z-10">
        <div className="mb-5 flex items-center justify-center">
          <Link href="/" className="inline-flex items-center transition duration-200 hover:-translate-y-0.5 hover:opacity-90">
            <span className="relative block h-7 w-[5.5rem]">
        ...
        <form className="mt-5" onSubmit={handleSubmit}>
          ...
          {errorMessage ? (
            <p className="mt-3 rounded-[10px] border border-danger/15 bg-danger/8 p-2.5 text-[0.76rem] font-semibold leading-5 text-danger">
              {errorMessage}
            </p>
          ) : null}
