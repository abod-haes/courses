...
import { Award, BookOpen, CheckCircle2, ClipboardCheck, GraduationCap, HeartPulse, Microscope, Ruler, ShieldCheck, Sparkles, Stethoscope, Target, Users2 } from "lucide-react";
...
type CardItem = Readonly<{
  title: string;
  description: string;
  icon: "science" | "skills" | "protocol" | "safety" | "book" | "ruler" | "users" | "target";
}>;
...
const contentByLocale: Record<Locale, AboutContent> = {
  ar: {
    ...
    subtitle:
      "منصة تعليمية رقمية تعيد صياغة التدريب على الطب التجميلي عبر العلم التشريحي، البروتوكولات الدقيقة، والمهارات العملية الآمنة تحت إشراف الدكتور إياس عكاري.",
    ...
    pillars: [
      {
        title: "العلوم التجميلية",
        description: "فهم تشريحي متعمق لفحص الجلد والبشرة والتاريخ الطبي لكل حالة قبل بناء التشخيص والخطة العلاجية.",
        icon: "science",
      },
      {
        title: "المهارات العملية",
        description: "تدريب على تقنيات الحقن والتعامل الآمن مع الإجراءات مع نقل أسرار تلافي الأخطاء والمضاعفات.",
        icon: "skills",
      },
      {
        title: "البروتوكولات العلاجية",
        description: "خطوات علاجية موثقة تساعد الطبيب على تحويل الممارسة إلى نظام واضح وقابل للتكرار بثقة.",
        icon: "protocol",
      },
    ],
    ...
    innovations: [
      {
        title: "مسطرة البوتوكس الهندسية",
        description: "أداة قياسية تساعد على ضبط وتوزيع الوحدات بدقة تناسب أبعاد كل وجه وتدعم نتائج طبيعية ومتوازنة.",
        icon: "ruler",
      },
      {
        title: "كتاب البروتوكولات العلاجية",
        description: "مؤلف طبي مرجعي يوثق فلسفة أن الجمال علم يدرس وله قواعد صارمة لا تخضع للعشوائية.",
        icon: "book",
      },
    ],
    ...
    why: [
      {
        title: "التحليل والتشخيص الأكاديمي",
        body: "دراسة نوعية الجلد، البشرة، والتاريخ الطبي لكل حالة كمسار منفرد لا كقالب جاهز.",
      },
      {
        title: "الأمان الطبي المطلق",
        body: "تعلّم أسرار تجنب الأخطاء والمضاعفات من مدرب ينقل خبرته العملية للأطباء.",
      },
      {
        title: "نتائج مضمونة وموثقة",
        body: "تحويل الممارسة الطبية إلى خطوات مدروسة تمنح الطبيب الثقة أمام مرضاه.",
      },
    ],
    closingTitle: "بوابتكم نحو التميز المهني العالمي",
    closingBody:
      "من خلال هذه المنصة، يضع الدكتور إياس عكاري خلاصة خبرته الأكاديمية والعملية بين يدي جيل الأطباء الجديد؛ لتكون IASS بيئة تعلم متكاملة تبني عقلية طبيب تجميل محترف يمتلك الثقة والمعرفة الهندسية التي تميزه في سوق العمل.",
  },
  en: {
    eyebrow: "About IASS",
    title: "International Academy of Aesthetic Science and Skills",
    subtitle:
      "A digital academy reshaping aesthetic medicine training through anatomical science, precise protocols, and safe practical skills under the guidance of Dr. Iyas Akkari.",
    ...
    pillars: [
      {
        title: "Aesthetic science",
        description: "Deep anatomical understanding for skin, facial structure, and patient history before diagnosis and treatment planning.",
        icon: "science",
      },
      {
        title: "Practical skills",
        description: "Training in precise injection techniques and safe procedures, with a focus on preventing errors and complications.",
        icon: "skills",
      },
      {
        title: "Treatment protocols",
        description: "Documented steps that turn clinical practice into a clear, repeatable system doctors can apply with confidence.",
        icon: "protocol",
      },
    ],
    ...
    innovations: [
      {
        title: "The engineering Botox ruler",
        description: "A measurement tool that helps control unit distribution according to each face's dimensions for natural balanced results.",
        icon: "ruler",
      },
      {
        title: "Aesthetic Treatment Protocols book",
        description: "A medical reference documenting the philosophy that beauty is a science with clear rules, not random practice.",
        icon: "book",
      },
    ],
    ...
    why: [
      {
        title: "Academic analysis and diagnosis",
        body: "Study skin type, facial structure, and medical history as an individual case, not a generic template.",
      },
      {
        title: "Absolute medical safety",
        body: "Learn how to avoid mistakes and complications from a trainer who teaches doctors practical safety standards.",
      },
      {
        title: "Reliable documented outcomes",
        body: "Turn clinical work into structured steps that give doctors confidence in front of their patients.",
      },
    ],
    closingTitle: "Your gateway to global professional excellence",
    closingBody:
      "Through IASS, Dr. Iyas Akkari places his academic and practical expertise in the hands of the next generation of doctors, building a complete learning environment for professional aesthetic practice.",
  },
};
...
function AnimatedSection({ children, className = "" }: Readonly<{ children: React.ReactNode; className?: string }>) {
...
function IconCard({ item }: Readonly<{ item: CardItem }>) {
  const Icon = iconByKey[item.icon] ?? CheckCircle2;

  return (
    <motion.article
      variants={itemVariants}
      className="group relative overflow-hidden rounded-[1.55rem] border border-border/70 bg-white p-5 shadow-[0_14px_34px_rgba(15,23,42,0.055)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_22px_48px_rgba(29,23,213,0.10)] dark:border-white/10 dark:bg-white/6 dark:shadow-none"
    >
...
export function AboutPage({ locale }: AboutPageProps) {
  ...
  return (
    <div className="overflow-hidden bg-section-bg">
      <section className="relative overflow-hidden px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,0.72fr)] lg:gap-14">
          ...
          <motion.div
            ...
            initial={shouldReduceMotion ? false : { opacity: 0, x: locale === "ar" ? -24 : 24, scale: 0.98 }}
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/44 p-2 shadow-[0_24px_70px_rgba(15,23,42,0.14)] backdrop-blur-xl dark:border-white/10 dark:bg-white/6">
              <div className="relative aspect-[0.86] overflow-hidden rounded-[1.55rem] bg-transparent">
                <Image
                  ...
                  className="object-contain object-bottom drop-shadow-[0_22px_32px_rgba(15,23,42,0.20)]"
                />
              </div>
      ...
      <AnimatedSection id="academy" className="px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-12">
          ...
          <motion.div variants={itemVariants} className="space-y-5 text-base leading-8 text-slate-600 dark:text-slate-300 sm:text-lg sm:leading-9">
            {content.academy.paragraphs.map((paragraph) => (
      ...
      <AnimatedSection className="px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="mx-auto max-w-7xl">
          <motion.div variants={itemVariants} className="mx-auto max-w-3xl text-center">
            <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary dark:bg-primary/18">
      ...
      <AnimatedSection className="px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="mx-auto max-w-7xl">
          <motion.h2 variants={itemVariants} className="text-3xl font-black tracking-tight text-slate-950 dark:text-white sm:text-4xl">
            {content.innovationsTitle}
          </motion.h2>
      ...
      <AnimatedSection className="px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="mx-auto max-w-7xl rounded-[2rem] bg-primary p-6 text-white shadow-[0_24px_70px_rgba(29,23,213,0.22)] sm:p-8 lg:p-10">
          <motion.h2 variants={itemVariants} className="text-3xl font-black tracking-tight sm:text-4xl">
            {content.whyTitle}
          </motion.h2>
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {content.why.map((item, index) => (
              <motion.div key={item.title} variants={itemVariants} className="rounded-3xl border border-white/16 bg-white/10 p-5 backdrop-blur-xl">
                <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/16 text-white">
      ...
      <section className="px-4 pb-16 pt-8 sm:px-6 lg:px-8 lg:pb-24">
        <div className="mx-auto max-w-4xl text-center">
          <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 text-primary dark:bg-primary/18">
          ...
          <div className="mt-8 flex justify-center gap-3">
            <Button href="/courses" className="rounded-2xl px-6">
              {content.primaryCta}
            </Button>
            <Button href="/articles" variant="secondary" className="rounded-2xl px-6">
              {locale === "ar" ? "اقرأ المقالات" : "Read articles"}
            </Button>
          </div>
        </div>
      </section>
