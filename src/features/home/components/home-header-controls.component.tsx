...
export function HomeHeaderControls({ copy }: HomeHeaderControlsProps) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-full bg-white/72 p-1 shadow-[0_10px_30px_rgba(15,23,42,0.07)] ring-1 ring-slate-200/70 backdrop-blur-xl dark:bg-white/10 dark:shadow-[0_12px_34px_rgba(0,0,0,0.22)] dark:ring-white/14">
      <button
        type="button"
        onClick={toggleTheme}
      ...
      >
        <span className="relative flex h-7 w-12 items-center rounded-full bg-slate-900/[0.06] p-0.5 transition duration-300 group-hover:bg-primary/12 dark:bg-white/16 dark:group-hover:bg-white/22">
          <span
      ...
      >
        ...
        <span className="text-xs tracking-[0.08em]">{languageLabel}</span>
      </button>
    </div>
  );
}
